...
const useLocalStorageBoolean = (key: string, initialValue: boolean): UseLocalStorageBooleanResult => {
  // Inicialize o estado com o valor do localStorage ou o valor inicial fornecido
  const [state, setState] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      const localStorageValue = localStorage.getItem(key)
      const localStorageValueChecker = localStorageValue ? JSON.parse(localStorageValue) : initialValue
